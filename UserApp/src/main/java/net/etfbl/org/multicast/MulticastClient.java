	...
	public void run() {
		...
		try {
			...
			while(true) {
				...
				System.out.println("Message user: " + recieved);
				
			}
			
		}catch(IOException e) {
			UserLogger.LOGGER.log(Level.SEVERE, "An error occurred while establishing a multicast connection.", e);
		}finally {
			try {
				socket.leaveGroup(address);
				socket.close();
			}catch (IOException e) {
				UserLogger.LOGGER.log(Level.SEVERE, "An error occured during socket closing.", e);
			}
		}
	}
